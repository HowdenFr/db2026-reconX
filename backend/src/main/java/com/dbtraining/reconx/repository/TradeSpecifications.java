 ...
 *          A null (or blank, for refLike) filter argument means "no
 *          constraint", encoded via cb.conjunction().
 * WHY:     Avoids exploding the repository with `findByXAndYAndZ...`
 *          methods for every possible combination of filters.
 * OBSERVE: GET /api/v1/trades?status=MATCHED&from=2026-01-01 should produce
 *          the right SQL WHERE clause — turn on `spring.jpa.show-sql` to verify.
 * ============================================================================
 */
